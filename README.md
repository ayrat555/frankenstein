...
```toml
frankenstein = { version = "0.29", default-features = false, features = ["async-http-client"] }
```

You can also disable all features:

```toml
frankenstein = { version = "0.29", default-features = false }
```
...
Frankenstein implements all telegram bot api methods. To see which parameters you should pass, check [docs.rs](https://docs.rs/frankenstein/0.29.1/frankenstein/api_traits/telegram_api/trait.TelegramApi.html#provided-methods)
...
```toml
frankenstein = { version = "0.29", default-features = false, features = ["telegram-trait"] }
```
